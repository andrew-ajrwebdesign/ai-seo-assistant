...
class Metadata_Generator {
	...
	public function generate( $post_id ) {
		...
		if ( is_wp_error( $result ) ) {
			$error_message = Utils::mask_sensitive_text( $result->get_error_message() );
	...
	public function generate_recommendations( $post_id ) {
		...
		$content_for_prompt = Utils::trim_to_length( $content, 8000 );
	...
	private function generate_ai_metadata( $post_id, $content ) {
		...
		$content = Utils::trim_to_length( $content, 6000 );
		...
		return [
			...
			'title_status'       => Utils::get_title_status( $title ),
	...
	private function generate_placeholder_metadata( $post_id, $content ) {
		...
		$title       = Utils::trim_to_length( $title, 60 );
		$description = Utils::trim_to_length( $best_sentence, 155 );
	...
	private function log_generation( $post_id, $result, $content, $source, $error = '' ) {
		$this->logger->add_log(
			...
			[
				...
				'content_preview' => Utils::trim_to_length( $content, 500 ),
				'error'           => Utils::mask_sensitive_text( $error ),
			]
		);
	}
}
